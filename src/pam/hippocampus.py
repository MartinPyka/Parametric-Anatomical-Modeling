...
c_dg_ca3, d_dg_ca3, s_dg_ca3, grid = pam.computeConnectivity(
    [dg, al_dg, ca3],                  # layers involved in the connection
    ...
    pam.connfunc_gauss_pre,            # kernel function plus parameters
    dg_params,
    pam.connfunc_gauss_post,
    ca3_params_dendrites,
    int(s_dg_ca3)                      # total syn f. ea. pre-syn neuron
)


if True:
    c_ca3_ca3, d_ca3_ca3, s_ca3_ca3, grid = pam.computeConnectivity(
        [ca3, al_ca3, ca3],                  # layers involved in connection
        ...
        int(s_ca3_ca3 * f)                   # total syn f. ea. pre-syn neuron
    )
...
pv.visualizeConnectionsForNeuron(
    [ca3, al_ca3, ca3],                  # layers involved in the connection
    ca3_neurons,                         # neuronsets involved
    ca3_neurons,
    1,                                   # synaptic layer
    [cfg.MAP_normal, cfg.MAP_normal],    # connection mapping
    [cfg.DIS_normalUV, cfg.DIS_euclid],  # distance calculation
...
pv.visualizeConnectionsForNeuron(
    [dg, al_dg, ca3],                  # layers involved in the connection
    dg_neurons,                        # neuronsets involved
    ca3_neurons,
    1,                                 # synaptic layer
    [cfg.MAP_euclid, cfg.MAP_euclid],  # connection mapping
    [cfg.DIS_euclid, cfg.DIS_euclid],  # distance calculation
