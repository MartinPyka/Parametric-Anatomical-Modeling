...
if __name__ == "__main__":
    ...
    c_ca3_ca3, d_ca3_ca3 = computeConnectivity(
        [ca3, al_ca3, ca3],  # layers involved in the connection
        'CA3_Pyramidal',     # neuronsets involved
        'CA3_Pyramidal',
        1,                   # synaptic layer
        [1, 0],              # connection mapping
        [1, 0],              # distance calculation
        connfunc_gauss,      # kernel function
        params
    )

    print('Compute Connectivity for ca3 to ca1')

    c_ca3_ca1, d_ca3_ca1 = computeConnectivity(
